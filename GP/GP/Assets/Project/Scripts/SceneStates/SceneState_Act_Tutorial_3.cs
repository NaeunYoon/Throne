...
public class SceneState_Act_Tutorial_3 : SceneStateBase
{
        ...
        if (GotoDirectNextState(sceneStatus) == false)
        {
            switch (sceneStatus)
            {
                case SceneBase.Status.FINISH_GOTO_WINTERFELL:
                    {
                        App.inst.sceneStateMgr.ChangeState<SceneState_Act_Winterfell>(SceneState_Act_Tutorial_3.MakeParam(GetType().Name, string.Empty));
                    }
                    break;
